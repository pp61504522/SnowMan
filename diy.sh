...
grep -q "jd_zjd" ${ListCron} && grep -q "jd_zjd.py" ${ListCron}
if [ $? -ne 0 ]; then
  sed -i "/&*jd_zjd/c7 * * * * export zlzh=[''] && python3 /jd/scripts/jd_zjd.py | " ${ListCron}
  sed -i 's#&*jd_zjd.py | #&tee /jd/log/jd_zjd/$(date "+%Y-%m-%d-%H-%M-%S").log#g' ${ListCron}
  cd ${ScriptsDir}
  pip3 install requests
  cd ${ShellDir}
fi

grep -q "jd_qjd" ${ListCron} && grep -q "jd_qjd.py" ${ListCron}
if [ $? -ne 0 ]; then
  sed -i "/&*jd_qjd/c0 9 * * * export qjd_zlzh=['',''] && python3 /jd/scripts/jd_qjd.py | " ${ListCron}
  sed -i 's#&*jd_qjd.py | #&tee /jd/log/jd_qjd/$(date "+%Y-%m-%d-%H-%M-%S").log#g' ${ListCron}
  cd ${ScriptsDir}
  pip3 install requests
  cd ${ShellDir}
fi
